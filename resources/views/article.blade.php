...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($_POST['txt']) && !empty($_POST['title'])) {
    ...
    // Query：データ追加
    $insert_query = "INSERT INTO `data`(`id`, `title`, `article`) VALUES ('{$id}', '{$title}', '{$text}')";
...
if (isset($_POST['del'])){
  ...
  $delete_query = "DELETE FROM `data` WHERE `id` = '{$_POST['del']}'";
      ...
      <?php foreach ((array)$BOARD as $DATA) : ?>
        ...
        <div>
            <form method="post">
              ...
              <input type="submit" value="投稿を消す" class="deleteComment">
            </form>
          </div>
